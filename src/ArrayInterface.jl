...
@inline function _to_cartesian(a, i::CanonicalInt)
    return map((sub, off) -> sub + off - oneunit(i), _ind2sub(size(a), i), offsets(a))
end
# modified from Base._ind2sub
@inline _ind2sub(sz::Tuple, i::CanonicalInt) = _ind2sub_recurse(sz,  i - oneunit(i))
@inline _ind2sub_recurse(::Tuple{}, i::CanonicalInt) = (i + oneunit(i),)
@inline _ind2sub_recurse(::NTuple{1}, i::CanonicalInt) = (i + oneunit(i),)
@inline function _ind2sub_recurse(sz::Tuple, i)
    d = first(sz)
    indnext = div(i, d)
    return (i - d * indnext + oneunit(i), _ind2sub_recurse(tail(sz), indnext)...)
end
